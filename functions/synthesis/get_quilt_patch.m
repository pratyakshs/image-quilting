function [ out_im ] = get_quilt_patch(patch, target, alpha, beta, Im, patch_size, overlap_size, i1, j1, send_t, mode)
...
%   Detailed explanation goes here
    ...
    elseif (i1 == 1)
        ...
        out_im = quilt_left_overlap(left_overlap, Im, patch_size, 200, target, alpha, beta, send_t, mode);
    elseif (j1 == 1)
        % We're in the left column, consider only the top overlap
        out_im = quilt_top_overlap(top_overlap, Im, patch_size, 200, target, alpha, beta, send_t, mode);
    else
        % In the interior, consider the L-overlap
        out_im = quilt_L_overlap(top_overlap, left_overlap, Im, patch_size, 200, target, alpha, beta, send_t, mode);
    end
end
